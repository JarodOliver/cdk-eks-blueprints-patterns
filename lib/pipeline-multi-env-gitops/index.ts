...
export default class PipelineMultiEnvGitops {
    ...
    async buildAsync(scope: Construct, id: string, pipelineProps: PipelineMultiEnvGitopsProps, props?: StackProps) {
        ...
        const devArgoAddonConfig = createArgoAddonConfig('dev', 'git@github.com:JarodOliver/cdk-eks-blueprints-patterns.git');
        const testArgoAddonConfig = createArgoAddonConfig('test', 'git@github.com:JarodOliver/cdk-eks-blueprints-patterns.git');
        const prodArgoAddonConfig = createArgoAddonConfig('prod', 'git@github.com:JarodOliver/cdk-eks-blueprints-patterns.git');

        try {

            // const { gitOwner, gitRepositoryName } = await getRepositoryData();
            const gitOwner = 'jarodoliver';
        ...
        argoAdditionalProject.push(
            {
                ...
                sourceRepos: [
                    ...
                    `git@github.com:JarodOliver/cdk-eks-blueprints-patterns.git`,
                ],
            }
        );
    });
